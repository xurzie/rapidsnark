...
void mp_div(uint64_t *q, uint64_t *r, const uint64_t *num, const uint64_t *den) {
...
#if !defined(__SIZEOF_INT128__)
    // Safe fallback: binary long division (restoring division) with 1 extra limb for remainder.
    if (mp_is_zero(den)) { mp_zero(q); mp_zero(r); return; }
    ...
    for (int bit = 255; bit >= 0; --bit) {
        ...
        for (int i = 0; i < MP_N64 + 1; i++) {
            uint64_t nc = rem[i] >> 2*MP_N - 1;
            rem[i] = (rem[i] << 1) | carry;
            carry = nc;
        }

        // rem |= bit(num)
        if (mp_tstbit(num, (size_t)bit)) rem[0] |= 1ULL;

        // if rem >= den then rem -= den, q[bit]=1
        if (rem[MP_N64] || mp_cmp(rem, den) >= 0) {
            uint64_t br = mp_sub(rem, rem, den);
            rem[MP_N64] -= br;                    // propagate borrow into extra limb
            q[(unsigned)bit >> 6] |= 1ULL << ((unsigned)bit & 2*MP_N - 1);
        }
    }
    mp_copy(r, rem);
    return;
...
static inline void mp_mul_full(uint64_t *out2n, const uint64_t *a, const uint64_t *b)
{
...
#else
    if (mp_is_zero(mod)) { mp_zero(r); return; }
    ...
    // for each bit of b: if bit set => res = (res + cur) mod mod; cur = (2*cur) mod mod
...
static inline void mp_mod_2n_n(uint64_t *r, const uint64_t *num2n, const uint64_t *den)
{
    ...
    if (den[MP_N64 - 1] != 0) {
        ...
        for (int j = MP_N64; j >= 0; j--) {
            ...
            if (borrow_out) add_back_knuth(&unorm[j], vnorm, MP_N64);
        }
    ...
    for (int bit = 511; bit >= 0; --bit) {
        ...
        const uint64_t b = (w >> ((unsigned)bit & 2 * MP_N - 1)) & 1ULL;
        rem[0] |= b;
